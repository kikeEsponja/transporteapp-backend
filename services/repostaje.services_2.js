...
const validarEstadoParaRepostaje = (servicio) => {
    if(servicio.estado_id !== estados.RECOGIDO){
        throw crearError(
            'Sólo se puede registrar un repostaje cuando el servicio está RECOGIDO',
            400
        );
    }
};
...
const crearRepostaje = async (idServicio, repostaje, idConductor) => {
    ...
    if(
        repostaje.litros === undefined ||
        repostaje.importe === undefined ||
        typeof repostaje.ticket !== 'string' ||
        repostaje.ticket.trim() === ''
    ){
        throw crearError(
            'Los litros, el importe y el ticket son obligatorios',
            400
        );
    }
    ...
    if(
        typeof repostaje.litros !== 'number' ||
        !Number.isFinite(repostaje.litros) ||
        repostaje.litros <= 0
    ){
        throw crearError(
            'Litros deben ser un número mayor que cero (0)',
            400
        );
    }
    ...
    if(repostaje.litros > 200){
        throw crearError(
            'La cantidad de litros no puede superar los 200',
            400
        );
    }
    ...
    if(repostaje.importe > 100){
        throw crearError(
            'El importe no puede superar los 100 euros',
            400
        );
    }

    if(
        typeof repostaje.importe !== 'number' ||
        !Number.isFinite(repostaje.importe) ||
        repostaje.importe <= 0
    ){
        throw crearError(
            'Importe debe ser un número mayor que cero (0)',
            400
        );
    }

    if(
        repostaje.observaciones !== undefined &&
        typeof repostaje.observaciones !== 'string'
    ){
        throw crearError(
    ...
    if(servicio.conductor_id !== idConductor){
        throw crearError(
            'No tienes permiso para registrar un repostaje de este servicio',
            403
        );
    }

    const repostajeExistente =
        await repostajesModel.obtenerRepostajePorTicket(
            idServicio,
            ticket
        );
